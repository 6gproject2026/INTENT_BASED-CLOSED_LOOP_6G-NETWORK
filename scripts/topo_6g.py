...
backup path (core-sp2-lf1) 10 Mbps. The shaped links pass cls=TCLink
explicitly, so no --link tc is needed and host links stay unshaped.
...
class SixGTopo(Topo):
    def build(self):
        ...
        # cls=TCLink per-link: bw= is only valid on TCLink, and passing it here
        # avoids depending on `--link tc` being remembered on the command line.
        self.addLink(core, sp1, cls=TCLink, bw=MAIN_BW)     # 13.0.0.0/24  main
        self.addLink(core, sp2, cls=TCLink, bw=BACKUP_BW)   # 14.0.0.0/24  backup
        self.addLink(sp1, lf1, cls=TCLink, bw=MAIN_BW)      # 15.0.0.0/24  main
        self.addLink(sp2, lf1, cls=TCLink, bw=BACKUP_BW)    # 16.0.0.0/24  backup
