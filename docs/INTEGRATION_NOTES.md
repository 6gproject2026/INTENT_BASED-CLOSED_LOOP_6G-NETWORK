...
*Note: We've fully migrated to live integration. The old offline mock environments have been wiped to keep things clean.*

## 1. Config checks (`prod.json`)
...
## 6. Real-world Pre-run Checklist

To avoid slamming into the same errors, run through this before starting:

1. Check that Ryu is actually running and reachable at the configured `base_url`.
2. Ensure Mininet topology is fully up and switches are populating in `/stats/switches`.
3. Check that the startup setup script goes through without routing/QoS failures.
4. Verify the latency endpoint is returning actual numbers, not nulls.
5. Kick off traffic generators so the telemetry has a heartbeat.
